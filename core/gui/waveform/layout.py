...
def waveform_layout(waveform_id, waveform_name, waveform, app):
    ...
    table = dbc.Col(
        html.Div([waveform_table(id=table_id, setpoints=waveform.return_SetPoints())]),
        style={"width": "150px"},
    )

    graph = dbc.Col(
        html.Div([waveform_plot(id=plot_id, setpoints=waveform.return_SetPoints())])
    )

    return_div = html.Details(
        [html.Summary(waveform_name), html.Div(dbc.Row([table, graph]))]
    )
